...
//
Shader "Hidden/Kvant/Spray/KernelAsteroidFieldSM"
{
    Properties
    {
        _PositionBuffer ("-", 2D) = ""{}
    ...
    float4 new_particle_position(float2 uv)
    {
		...
		//v0.1 - push away from center
		//p = p * 211 *0;
		//move to center
		//p = (p - float3(1,1,1)*0.5);
		p = (p - float3(1, 1, 1)*0.5);
		float dist = sqrt(p.x*p.x+p.z*p.z);		
		//float3 normVec = p / dist;
		//p = normVec * 1;//
		//p = (p + float3(1, 1, 1)*0.5);
		p =  (p) / (dist);// +float3(p.x, p.y, p.z) * 1 * (1 / (dist*dist + 0.1));
		//move to origin
		p = (p + (float3)0.5);

...
namespace Artngame.Orion.PlanetCreator.Atmosphere
{
    ...
    public class AtmosphericScatter : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (imageFXBlend < 1)
            {
                ...
                cameraA.AddCommandBuffer(cameraEvent, lightingBuffer);
            }
            else
            {
                //camera.AddCommandBuffer(CameraEvent.BeforeLighting, lightingBuffer);
                GetComponent<Camera>().AddCommandBuffer(cameraEvent, lightingBuffer);
            }

            cameraA.depthTextureMode = DepthTextureMode.DepthNormals;
        ...
        void Update()
        {
            ...
            cameraA.farClipPlane = Mathf.Max(MinViewDistance, (transform.position - transform.position.normalized * (RG * SCALE / 3f)).magnitude);
        ...
        private void UpdateRenderBuffer()
        {
            ...
            Matrix4x4 P = cameraA.projectionMatrix;
            Vector4 CamScreenDir = new Vector4(1f / P[0], 1f / P[5], 1f, 1f);
            lightingBuffer.SetGlobalVector("_CamScreenDir", CamScreenDir);
            Matrix4x4 viewProjInverse = (cameraA.projectionMatrix * cameraA.worldToCameraMatrix).inverse;
            lightingBuffer.SetGlobalMatrix("_ViewProjectInverse", viewProjInverse);
            Matrix4x4 CameraInv = cameraA.cameraToWorldMatrix.inverse;
            lightingBuffer.SetGlobalMatrix("_CameraInv", CameraInv);
            lightingBuffer.SetGlobalMatrix("_ViewMatrix", cameraA.worldToCameraMatrix);
            lightingBuffer.SetGlobalFloat("_FarPlane", cameraA.farClipPlane);
            ...
            lightingBuffer.GetTemporaryRT(downScaleTex, cameraA.pixelWidth / Mathf.Max(1, downscale), cameraA.pixelHeight / Mathf.Max(1, downscale), 0, FilterMode.Bilinear, RenderTextureFormat.ARGBFloat);
            if (imageFXBlend < 1)
            {

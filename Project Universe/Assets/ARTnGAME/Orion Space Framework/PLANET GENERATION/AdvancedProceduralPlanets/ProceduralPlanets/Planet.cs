...
namespace Artngame.Orion.PlanetGEN
{
    ...
    public partial class Planet : MonoBehaviour, IDisposable
    {
        ...
        void OnGUI()
        {
            if (showGUI)
            {
                GUI.TextArea(new Rect(10, 10 + 100, 300, 40), currentStep);
            }
        }
        ...
        void LateUpdate()
        {
            //if (Time.fixedTime - lastTime > updateEvery)
            {
        ...
        private void GenerateChunks(Stopwatch frameStart, TimeSpan timeBudget)
        {
            ...
            if (chunkGenerationCoroutinesInProgress.Count <= 30)
            {
            ...
            //do
            {
                for (int i = chunkGenerationCoroutinesInProgress.Count - 1; i >= 0; --i)
                {
                    ...
                    //if (frameStart.Elapsed.Seconds > 0.0001f) break;
                }
            }

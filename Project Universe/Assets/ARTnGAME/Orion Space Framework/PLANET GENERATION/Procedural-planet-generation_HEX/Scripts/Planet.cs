...
namespace Artngame.Orion.PlanetGenerationHEX
{
    public class Planet : MonoBehaviour
    {
        ...
        public static List<GameObject> planetListOBJS = new List<GameObject>();
        ...
        static private void DataThreadLoop()
        {
            ...
            lock (planetList)
            {
                foreach (var planet in planetList)
                {
                    ...
                    //Debug.Log("InitAsIcosahedron");
                    // Subdivide it.
                    planet.Subdivide();
                   // Debug.Log("Subdivide");
                    // Create noise offset.
                    planet.CalculateOffset();
                    //Debug.Log("CalculateOffset");
        ...
        static private bool PolyInRange(Polygon poly, Vector3 position)
        {
            ...
            return d0 < G_viewDistance || d1 < G_viewDistance || d2 < G_viewDistance;
        }
        /// <summary>
        ...
        static public void AddPlanetToQueue(string name, Vector3 position, int seed, bool randomSeed, 
            Style terrainStyle, float seaLevel, GenerationData[] generationData, ColorHeight[] colorHeight, 
            PopulateData[] population, Material material, Material seaMaterial, float radius, int subdivisions, int chunckSubdivisions, float scaler)
        {
            // Instantiate a gameobject that hold planet.
            GameObject obj = new GameObject();
            obj.name = name;
            obj.transform.position = position;
          //  obj.transform.localScale = scaler * Vector3.one;

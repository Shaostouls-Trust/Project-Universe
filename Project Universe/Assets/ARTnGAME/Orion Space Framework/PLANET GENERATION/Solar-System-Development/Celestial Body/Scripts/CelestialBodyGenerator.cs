    ...
    public class CelestialBodyGenerator : MonoBehaviour
    {
        ...
        void HandleGameModeGeneration()
        {
            if (CanGenerateMesh())
            {
                ...
                // Add collider
                MeshCollider collider = terrainHolder.GetComponent<MeshCollider>();
                if (collider == null)
                {
        ...
        Vector2 GenerateTerrainMesh(ref Mesh mesh, int resolution)
        {
            ...
            for (int i = 0; i < crudeTangents.Length; i++)
            {
                tangs.Add(crudeTangents[i]);
            }

Shader "ORION/Scattering/Atmospheric Scattering Only Atmos HDRP FAR INNER"
{
	Properties
	{
		...
		_passSunExternally("Give sun pos-color with globalSunPosition-SunColor", float) = 0
			...
			//v0.3
			discardAboveBrightness("discard Above Brightness", float) = 0
	}
		SubShader{
			...
			Zwrite Off
				...
				float4 LightingScatteringA(float3 normal, float3 viewDir, float3 lightDir)
				{
					...
					//v0.1
					float dist = length(_WorldSpaceCameraPos - planetCenter) - _PlanetRadius / 4;
					viewDir = viewDir * 0.1 / (dist / atmosphereRadius);
					...
					//v0.1
					//col.a = 1.2 * col.a*cosTheta/(col.r*col.g*col.b*30.1);	
					dist = length(_WorldSpaceCameraPos - planetCenter) - _PlanetRadius / 4;
					col.a = saturate(1.2 * col.a * pow(cosTheta, 1) + 0.000000005 / pow(dist / atmosphereRadius, 8));// -dist * 0.001);
				...
				float4 frag(v2f IN) : SV_Target{
					...
					float factor = normalize(dot(IN.normal, cameraDir));
					if ((factor) >=0){// == 1) {
						
						//v0.1
						planetCenter = mul(unity_ObjectToWorld, half4(0, 0, 0, 1));
						planetCenter = float4(planetCenter.x / _SphereRadius, planetCenter.y / _SphereRadius, planetCenter.z / _SphereRadius, 1);
						worldCentre = float3(planetCenter.xyz) * (_SphereRadius - 1); //worldCentre = -float3(planetCenter.xyz) * (_SphereRadius - 1);// float3(0, 0, 0);//v0.1

						worldPos = IN.worldPos;
						UnitsToMetres = _PlanetRadius / _SphereRadius;
						//	planetCenter = _PlanetRadius * planetCenter;		//v0.1
						...
						planetCenter = mul(unity_ObjectToWorld, half4(0, 0, 0, 1));
						//planetCenter = float4(planetCenter.x / _SphereRadius, planetCenter.y / _SphereRadius, planetCenter.z / _SphereRadius, 1);
						//worldCentre = float3(planetCenter.xyz) * (1); //worldCentre = -float3(planetCenter.xyz) * (_SphereRadius - 1);// float3(0, 0, 0);//v0.1

						//if (length(_WorldSpaceCameraPos - planetCenter) < _SphereRadius*2) {
						if (length(_WorldSpaceCameraPos - planetCenter) < _SphereRadius * 2) {
							//_SphereRadius = _SphereRadius + length(_WorldSpaceCameraPos - planetCenter) - _SphereRadius / 2;// 250000;
							_SphereRadius = length(_WorldSpaceCameraPos - planetCenter) * 1.04;// 250000;
						}
						...
						//_SphereRadius = _SphereRadius + 200000;

						_PlanetCentre = planetCenter.xyz / _SphereRadius;
						spacePos = UnitsToMetres * (worldPos - planetCenter);
						atmosphereRadius = (_AtmosphereHeight * _AtmosphereModifier) + _PlanetRadius;
						_MScaleHeight *= _AtmosphereModifier;
						_RayScaleHeight *= _AtmosphereModifier;
						_MScatteringCoefficient *= _ScatteringModifier;
					...
					if (factor >= 0) {// == 1) {
						scatter = LightingScattering(IN.normal, _LightDirectionFX.x * IN.viewdir + _LightDirectionFX.yzw, _WorldSpaceLightPos0);
					}
					else {
						scatter = LightingScatteringA(IN.normal, _LightDirectionFX.x* IN.viewdir + _LightDirectionFX.yzw, _WorldSpaceLightPos0);
					}
					...
					//v0.1
					float4 outPut = float4(pow(scatter.rgb, 1) * 1, cAlbedo.a * scatter.a);
					if (discardAboveBrightness != 0) {
						if (outPut.r > discardAboveBrightness && outPut.g > discardAboveBrightness && outPut.b > discardAboveBrightness) {
							//outPut = float4(0,0,0,0);
							discard;
						}
					}

					return outPut;// float4(pow(scatter.rgb, 1) * 1, cAlbedo.a * scatter.a);
				}
				ENDCG
			}
			////END PASS 2

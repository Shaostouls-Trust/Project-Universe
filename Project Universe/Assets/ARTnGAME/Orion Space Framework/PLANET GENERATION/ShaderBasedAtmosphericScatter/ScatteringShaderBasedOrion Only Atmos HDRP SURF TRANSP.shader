Shader "ORION/Scattering/Atmospheric Scattering Only Atmos HDRP SURF TRANSP"
{
	Properties
	{
		...
		SubShader{
			...
			Tags{ "RenderType" = "Transparent" 	"Queue" = "Transparent" }
				...
				Pass{
					...
					v2f vert(appdata_full v) {
						...
						float3 viewDir = normalize(worldPos - 1 * _WorldSpaceCameraPos.xyz);// +float3(0, -_SphereRadius, 0));
				...
				float4 LightingScattering(float3 normal, float3 viewDir, float3 lightDir)
				{
					...
					float tA;
					float tB;
					if (!rayInstersection(UnitsToMetres * (_WorldSpaceCameraPos - worldCentre), viewDir, _PlanetCentre, atmosphereRadius, tA, tB)) {
					...
					float pA, pB;
					if (rayInstersection(UnitsToMetres *_WorldSpaceCameraPos, viewDir, _PlanetCentre, _PlanetRadius, pA, pB))
					{
						tB = 1 * pA;
					}
					...
					{
						float3 P = UnitsToMetres * _WorldSpaceCameraPos*1.0 +1*viewDir * (time*1 + 0.5*viewSampleSize);
						float height = distance(P, _PlanetCentre) - _PlanetRadius;
						//height = 1 / height;
						//height = atmosphereRadius - height;
						//height = height*0.1;
			...
			//		spacePos = _WorldSpaceCameraPos;	
					...
					return float4(pow(scatter.rgb,1) * 1, cAlbedo.a * scatter.a -0.3* (0.3-scatter.r) );
				}
				ENDCG
			}
			////END PASS 2

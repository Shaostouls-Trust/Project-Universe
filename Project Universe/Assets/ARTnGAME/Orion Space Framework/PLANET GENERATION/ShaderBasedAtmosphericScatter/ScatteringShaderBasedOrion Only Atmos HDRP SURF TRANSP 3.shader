Shader "ORION/Scattering/Atmospheric Scattering Only Atmos HDRP SURF TRANSP 3"
//Shader "ORION/Scattering/Atmospheric Scattering Only Atmos HDRP SURF TRANSP 1"
{
	Properties
	{
		...
		_passSunExternally("Give sun pos-color with globalSunPosition-SunColor", float) = 0
			...
			discardAboveBrightness("contrast discardAboveBrightness", float) = 0
	}
		SubShader{
				...
				Pass{
					...
					float4 frag(v2f IN) : SV_Target{
						...
						if (length(_WorldSpaceCameraPos - planetCenter) < _SphereRadius * 2) {
							...
							//v0.1
							float tranp = 1;
							if (scatter.r <= 0.14 && scatter.g <= 0.14 && scatter.b <= 0.14) {
								//tranp = saturate((scatter.r * scatter.b* scatter.b)*3510 + 0.5);
							}
							...
							//v0.1
							float4 outPut = float4(pow(scatter.rgb, 1) * 1, cAlbedo.a * scatter.a * tranp*1);

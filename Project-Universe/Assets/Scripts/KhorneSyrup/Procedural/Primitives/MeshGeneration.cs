...
public class MeshGeneration : MonoBehaviour
{
        ...
        public void GenerateCollider(bool collider, int meshCase)
    {
        if (collider && meshCase == 0)
        {
        ...
        else if (collider && meshCase == 1)
        {
            if (gameObject.GetComponent<MeshCollider>())
            {
                DestroyImmediate(gameObject.GetComponent<MeshCollider>());
                GenerateCollider(hasCollider, 0);
            }
        }
        else if (!collider)
            {
        ...
        GenerateCollider(hasCollider, 1);
        }
